...
impl Bfs {
    ...
    /// Runs the Breadth-First Search algorithm on a graph (starting from an optional start node).
    ...
    ///
    /// let bfs = Bfs::new();
    /// let mut trees = bfs.run(&mut graph, None);
    /// for (i, tree) in trees.iter_mut().enumerate() {
    ///     tree.svg_layout();
    ///     let html = tree.svg_render_animation_html("BFS");
    ///     let html_file_name = format!("example-Travers-BFS_tree{}.html",i);
    ///     write!(File::create(html_file_name).unwrap(), "{}", html).unwrap();
    /// }
    /// let html = graph.svg_render_animation_html("traversal BFS example");
    /// write!(File::create("example-Travers-BFS.html").unwrap(), "{}", html).unwrap();     
    /// ```
    ...
    fn bfs_nodes(
        ...
        if !nodes_next.is_empty() {
            self.bfs_nodes(nodes_next, graph, visited, tree);
        }
    }
}
